...
begin
  ...
  values (
    ...
    v_room.id,
    case
      when v_target_player_id is null then '[]'::jsonb
      else jsonb_build_array(v_target_player_id)
    end,
    'private'
  )
  returning game_events.id into v_game_event_id;

  insert into public.game_event_visible_players (game_event_id, player_id)
  values (v_game_event_id, v_player.id);
