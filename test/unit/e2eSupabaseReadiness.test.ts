...
} from "../../scripts/e2eSupabaseReadiness.mjs";
...
describe("E2E Supabase Realtime readiness", () => {
  ...
  it("reports the last gateway response when readiness times out", async () => {
    ...
    ).rejects.toThrow("Local Supabase Realtime was not ready within 50ms. Last attempt: HTTP 502");
    expect(probe).toHaveBeenCalledTimes(4);
  });

  it("probes the public gateway with the same WebSocket route used by browsers", async () => {
