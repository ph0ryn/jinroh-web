...
Waiting では着席進捗と開始条件を常に表示する。Portrait layout と Phone Landscape では
Room code、copy、share、QR code を単一の招待 modal にまとめ、操作領域には modal を
開く action を表示する。Portrait layout では着席進捗と招待 action を同じ行へ等幅で配置する。
着席進捗は単一の progress bar で表現し、Settings action は Host 操作 panel 内に置く。
Tablet Landscape / Desktop では同じ招待情報を操作領域内へ直接表示する。
