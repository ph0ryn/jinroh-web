...
最初のマイルストーンでは、実際の人狼ゲームロジックに入る前の基盤に集中する。
...
実際のゲームロジックはこの時点以降に始まり、現在の MVP の範囲外である。
ゲームロジックの設計は `docs/game/` 以下に分けて記録する。
...
Room、Player、event の物理削除は MVP の振る舞いに含めない。
...
ルーム画面はルーム変更通知を購読し、通知を受け取った後にルーム状態を
再読み込みする。
