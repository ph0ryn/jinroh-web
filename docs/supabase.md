...
- `docs/game/night-conversation.md`
- `docs/game/model.md`
...
- night conversation display view
...
- internal account ID
- nullable current Room reference
- created time
- updated time
- deleted time
...
- `current_room_id` は Account が現在入室している Room の唯一の正本
- `current_room_id` は同時に最大 1 つだけ保持できる nullable FK。Room の物理削除時は
  `on delete set null` とし、FK 自体も `deferrable initially deferred` とする
- `joined` / `disconnected` は接続状態であり、Room membership の正本として使わない
- 完了したゲームの `ended` は `current_room_id` を解除しない
- 明示退出または開始前 Room の `waiting` から `ended` への transition で
  `current_room_id` を解除する
- Browser の Room code 保存値から `current_room_id` を復元または上書きしない

`accounts` は game logic の主体ではない。game logic は必ず `players` を主体にする。
ただし、Account が現在どの Room に入室しているかの判定だけは
`accounts.current_room_id` を使う。
...
- `current_room_id is not null` の Account の token は cleanup しない
...
- `ended` の古い Room の code は再利用してよい
- Room は `waiting` で作成する
- target player count は 3 から 10 の整数
- `waiting` Room は30分以内に開始されなければ期限切れ
- 期限切れの Room は物理削除せず `ended` にする
- `waiting` host が退出した場合は最初に参加した残存 Player の Account に host を移譲する
- `waiting` Room は最後の Player が退出した場合に `ended` にする
- `ended` かつ `started_at is null` は開始前終了、`ended` かつ対応する
  `game_states.status = ended` は完了ゲームを表す
...
- 新しい Player は Room が `waiting` の間だけ作れる
- 既存 Player は `waiting` または `playing` で再参加できる
- Player display name は新規 Player 作成時だけ snapshot する
- 同じ Account が同じ Room に戻る場合、既存 Player を再利用する
- Player status は Room 内の接続状態と参加履歴であり、Account の現在 Room の
  正本として使わない
...
- Account を認証する
- Account を lock し、`current_room_id is null` を確認する
- Room code を発行する
- random realtime topic を発行する
- Room を `waiting` で作成する
- host Account を保存する
- target player count を保存する
- host Player を作成する
- Account の `current_room_id` を作成した Room に設定する
- display name を host Player に snapshot する
- waiting expiration time を設定する
- ゲーム開始前は `game_states` を作成しない
...
- Account を lock する
- waiting 期限切れを確認し、必要なら `ended` にする
- `current_room_id is null` なら参加先 Room に設定する
- `current_room_id` が参加先 Room と同じなら既存 Player を再利用する
- `current_room_id` が別 Room なら `current_room_exists` として拒否する
- 新規 Player または `left` / `disconnected` Player の再参加で target player count を超えないことを確認する
- 新規 Player は `waiting` の間だけ作成する
- 既存 Player は `waiting` または `playing` で再参加させる
...
`app_get_current_room` は認証済み Account の `current_room_id` から Room を取得する。

- `current_room_id is null` なら Room なしとして返す
- current Room が期限切れの `waiting` Room なら同じ transaction で `ended` にし、
  `current_room_id` を解除して Room なしとして返す
- Account に対応する `left` ではない Player が current Room に存在することを確認する
- Account ID や内部 Room ID は Browser-facing result に含めない

Application server の `GET /api/rooms/current` はこの transaction の結果から
`{ room: RoomSummary | null }` を返す。

### Room Switch

`app_switch_create_room` と `app_switch_join_room` は、UI で明示的な退出確認が
完了した場合だけ application server から呼ぶ。

同じ transaction で行う。

- Account を認証する
- request の `expectedCurrentRoomCode` が Account の current Room と一致することを
  確認する。一致しなければ `current_room_changed` として拒否する
- 既存の source / target Room を内部 Room ID 昇順に lock し、次に Account ID 昇順、
  Player ID 昇順で lock する
- source Room が `playing` なら `room_switch_forbidden` として拒否する
- source Room が `waiting` または完了ゲームの `ended` であることを確認する
- target の期限、status、定員と既存 Player を検証してから source を変更する
- source Player を `left` にし、host 移譲または待機 Room の終了を行う
- target を作成または参加し、Player を作成または再利用する
- Account の `current_room_id` を target Room に更新する
- source / target の event と notification result を作る

target が存在しない、期限切れ、満員、参加不可、または同時操作と競合した場合は
transaction 全体を rollback し、source Room の Player、host、event と
`current_room_id` を変更しない。

transaction が返す stable error marker は `current_room_exists`、
`current_room_changed`、`room_switch_forbidden`、`room_not_found`、
`room_expired`、`room_full`、`room_not_joinable` とする。

Application server の `POST /api/rooms/switch` は `kind = create | join` と
`expectedCurrentRoomCode` を受け取り、成功時は target の `RoomSummary` を返す。
...
- host Account か確認する
- Room を lock する
- Room が `waiting` か確認する
- waiting 期限切れでないことを確認する
- joined Player 数が target player count と一致することを確認する
- RuleSet を検証する
- `game_states` を `assigning_roles` で作成して lock する
...
- Player を `left` にする
- Account の `current_room_id` を `null` にする
- `player_left` event を記録する
- Room が `waiting` で、host が退出した場合は、`joined_at`, Player ID の順で
  最初の `joined` / `disconnected` Player の Account に host を移譲する
- Room が `ended` の場合も、残存 Player がいる間は同じ規則で host を移譲する
- Room が `waiting` で、残存 Player がいない場合は Room を `ended` にする
- 開始前に終了した場合は `room_ended` event も記録する
...
- Account を認証する
- public room code で Room を取得して lock する
- Account を lock し、Room が `current_room_id` と一致することを確認する
- waiting 期限切れを確認し、必要なら `ended` にする
...
- `rooms.waiting_expires_at <= now`
- 条件を満たす場合は `ended` にする
- 対象 Room を `current_room_id` とする Account を `null` に更新する
- `room_ended` event を記録する

scheduled cleanup はあってもよいが、正しさの入口にしない。
`app_cleanup_expired_waiting_rooms` は、放置された expired waiting Room を batch で
`ended` にし、`room_ended` event を記録する maintenance 用入口である。
...
- Room ごとに高々1行
- Room が `waiting` の間は game state を持たない
- game start transaction 内で `assigning_roles / phase = null` として作成する
...
- `assigning_roles` / `ended` では `phase` は null
...
- game status と Room status がともに `ended` になる前は、他人の role assignment を public
  view に含めない
- game status と Room status がともに `ended` になった後は、固定済み role assignment から
  全 Player の Role ID を result view に含められる
- game status と Room status がともに `ended` になる前の自分の role は self private view に
  だけ含める
...
- disconnect / leave は transport / connection の状態
...
夜会話や role group secret は、Team ではなく Role ID または Player ID で
閲覧対象を切る。狂人は `Team.Werewolf` でも WerewolfRole ではないため、
Werewolf night conversation の閲覧対象に含めない。
...
- message は append-only
- message body は trim 後 1 文字以上 100 文字以内
- group ID は resolved role setup の `nightConversationGroups` に存在するものだけ許可する
- sender の Role が group の `roleIds` に含まれる場合だけ送信できる
- Night 中だけ送信できる
- Night 以外では読み取り専用で見せられる
- realtime payload には message body を入れない

v1 の Werewolf night conversation の閲覧対象は実際に WerewolfRole を持つ Player だけ。
...
- Room
- selected RuleSet
- waiting Room の Players

処理。

- Room row を lock する
- Room が `waiting` で期限切れでないことを確認する
...
- request の expected revision が現在の `game_states.revision` と一致する
...
- `phase_instance_id` と expected `revision` を検証する
...
- 新規に受理した submission だけ、`action_submitted` event を `private` で記録する
- `action_submitted` の visible player は実際の submitter だけに限定し、action key、
  action kind、phase instance ID を本人用 receipt として保持する
- 早期解決条件を満たす場合、phase resolution を続けて実行する

二重送信。

- 同じ current action への2回目以降の有効 submission は no-op とする
- no-op では新しい `action_submitted` receipt を作成しない
- no-op でも成功扱いで現在 view を返してよい
- 無効 submission は pending action を作らず、current action を完了させない

### Night Conversation Message Send

夜会話は game action ではない。
そのため、`current_actions` / `pending_actions` ではなく
`night_conversation_messages` への append として扱う。
...
- request の night number が現在のものと一致することを確認する
- sender Player の Role を `role_assignments` から取得する
- request の conversation group ID が `resolved_role_setup.nightConversationGroups` に存在することを確認する
- sender Role が group の `roleIds` に含まれることを確認する
- message body を trim し、1 文字以上 100 文字以内であることを検証する
- `night_conversation_messages` に1行 insert する
- `game_states.revision` を増やす
- private view changed の notification result を返す

拒否する request。

- Day / Voting / Execution 中の送信
- stale phase instance ID
- stale night number
- 未定義の conversation group ID
- sender Role が group に含まれない送信
- 空 message
- 100 文字を超える message
...
- Account の `current_room_id` は維持する
...
- game status と Room status がともに `ended` になった後の Player ごとの Role ID

含めないもの。

- Account ID
- game status と Room status がともに `ended` になる前の role assignment
- game status と Room status がともに `ended` になる前の他人の role
- night action target
- inspection result
- attack target
...
- WerewolfRole の Player 向け night conversation
...
- Night 以外では night conversation を読み取り専用で見せられる
- sender、message body、timestamp だけを message として返す
...
- attack target
- night conversation internal sender reference
...
- `accounts.current_room_id` nullable FK to `rooms.id` with `on delete set null`,
...
- transaction 終了時、`accounts.current_room_id` が非 null なら同じ Account と Room の
  `left` ではない Player が1件だけ存在する
- `accounts.current_room_id` は Room membership の唯一の正本とし、Player status から
  membership を逆算または同期しない
- current Room は `waiting`、`playing`、または完了ゲームの `ended` である

複数 table の current Room 整合性は deferred constraint trigger で検証し、
create、join、leave、switch、waiting Room 終了 transaction 内の一時的な中間状態は
許容する。Room が開始前に `waiting` から `ended` へ変わる通常の `after` trigger は、
該当 Room を参照する `accounts.current_room_id` を即時 `null` にし、すべての expiration
経路を同じ規則に収束させる。完了ゲームの `playing` から `ended` への transition では
membership を維持する。
...
- night conversation body is 1 to 100 characters
- night conversation group ID has a safe text shape
...
- `rooms.status + rooms.waiting_expires_at`
- `rooms.host_account_id`
- `accounts.current_room_id`
- `players.room_id`
- `players.account_id`
...
- `night_conversation_messages.room_id + conversation_group_id + night_number + created_at`
- `night_conversation_messages.sender_player_id`
...
- active room code: `rooms.public_room_code` where status in `waiting`, `playing`
...
- `current_room_id is not null` の Account は cleanup しない
- `current_room_id is not null` の Account の token は revoke しない
- `current_room_id is null` の Account は cleanup 対象にできる
...
## Single Room Membership Migration

既存 schema へ `accounts.current_room_id` を追加するときは、次の順で移行する。

1. nullable column、Room FK、index を追加する
2. Room status が `waiting`、`playing`、`ended` で、Player status が `joined` または
   `disconnected` の row を current Room 候補として抽出する
3. 同じ Account に複数候補があれば Room を自動選択せず
   `single_room_membership_backfill_conflict` で migration を失敗させる
4. 候補が1つの Account だけを backfill する
5. deferred constraint trigger を有効にする
6. create、join、leave、heartbeat、expiration、cleanup、switch RPC を同じ migration
   で置き換え、旧 RPC と新 constraint が不整合な状態を公開しない

Room、Player、event の履歴は削除しない。未リリース環境で重複候補が見つかった場合は、
原因を確認したうえで disposable database を reset して migration を再検証する。
...
2. Account current Room と Room create / join / leave / switch / waiting expiration を
   transaction 化する
...
8. night conversation messages と ordered speech slot を作る
...
- Account ID を Browser へ返さない
- Account の現在 Room は `accounts.current_room_id` だけから判定する
- 同じ Account に複数の current Room を成立させない
- 完了ゲームの `ended` では current Room を解除せず、明示退出で解除する
- 開始前に `waiting` から `ended` になった場合は current Room を解除する
- switch 失敗時は source Room の所属を維持する
- Player ID で認可しない
...
- night conversation を対象 Role group 以外に返さない
