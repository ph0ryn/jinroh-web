...
- `joined` / `disconnected` と Room の `ended` は `current_room_id` を解除しない
- 明示退出または Room の `disbanded` transition だけが `current_room_id` を解除する
...
- current Room が期限切れ lobby なら同じ transaction で `disbanded` にし、
...
- source Room が `lobby` または `ended` であることを確認する
- target の期限、status、定員と既存 Player を検証してから source を変更する
- source Player を `left` にし、host 移譲または lobby の disband を行う
...
- transaction 終了時、`accounts.current_room_id` が非 null なら同じ Account と Room の
  `joined` または `disconnected` Player が1件だけ存在し、Room status は `lobby`、
  `playing`、`ended` のいずれか
- transaction 終了時、`lobby`、`playing`、`ended` の Room に `joined` または
  `disconnected` Player が存在する場合、その Account の `current_room_id` は同じ
  Room であり、他の active Room に active Player が存在しない

複数 table の current Room 整合性は deferred constraint trigger で検証し、
create、join、leave、switch、disband transaction 内の一時的な中間状態は許容する。
Room が `disbanded` へ変わる通常の `after` trigger は、該当 Room を参照する
`accounts.current_room_id` を即時 `null` にし、すべての expiration / disband 経路を
同じ規則に収束させる。
...
2. Room status が `lobby`、`playing`、`ended` で、Player status が `joined` または
...
2. Account current Room と Room create / join / leave / switch / lobby expiration を
   transaction 化する
...
- `ended` では current Room を解除せず、明示退出または `disbanded` で解除する
