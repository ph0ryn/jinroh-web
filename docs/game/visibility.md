...
この aggregate も無制限な row dump ではなく、exact key を持つ明示的な v2
projection とする。Room、lobby Player、nullable な current Game aggregate を分け、
Game aggregate 内の全 row は `Room.currentGameId` で絞る。通常の browser-facing read は
engine history を要求せず、`resolvedActions` を空のまま読む。完全な履歴を opt in するのは phase resolver
...
- current Game status が `ended` になる前は、public game view に他人の役職を
  含めない
- public game view に夜 action の対象を含めない
- realtime message に秘密情報を含めない
- ゲーム終了前の自分の役職は self private view にだけ含める
- current Game status が `ended` になった後は、その Game を current view として持つ
  active Room member に限り、固定済み game roster から全 Player の役職を public result
  view に含められる
- active member でない Room lookup は completed current Game を投影せず、役職、event、
  night conversation、receipt、winner、PlayerResult を返さない
- `Room.currentGameId = null` の snapshot は Game aggregate、自分の Role、alive state、
  action、event、night conversation、result を一切持たない
...
夜会話の通知は対象 Game と group の Role を持つ Player だけが購読できる private topic に送る。
人狼側の Team ID claim だけで購読を許可しない。

Role-private topic、grant 発行時 Game、Room current Game、Game Player assignment が同じ
Game ID を指すことを realtime authorization で確認する。過去 Game の grant または
topic は、Game pointer が外れた直後から current role view の購読に使えない。
