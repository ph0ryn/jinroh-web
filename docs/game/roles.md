...
common engine に特定の role id や role-owned action kind を直接分岐として追加しない。
処刑、投票、死亡、inspection、attack、protection などの generic capability / effect / resolution
concept は common に置けるが、それを発生させる role action identifier と semantics は Role が所有する。
既存の hook で予期できない役職を表せないときは、その役職を検出する例外ではなく、
同種の将来役職も使える generic hook や contract を追加する。

役職ごとの module は、その役職の identifier、action semantics、target rule、
metadata、option、localized fallback、終了理由、winner judgement を所有する。
registry と bootstrap 以外の production code は、role-owned identifier を列挙しない。
...
- action の対象候補をどう決めるか
- first night 開始時にどの informational result / message を通知するか
- 夜会話 group に参加するか
- 勝者判定にどの priority の judgement を追加するか
- その Role が所有するゲーム終了条件を満たすか
...
Team は shared enum ではなく、Role が ID と localized presentation を提供する
opaque な定義。`RoleRegistry` は登録された Role definitions の Team を ID ごとに
集約し、同じ ID を共有する Role の presentation が一致することを検証する。winner judgement は
登録済み Team ID だけを返し、UI は registry 由来の team catalog を表示する。

妖狐は `"fox"` Team ID を持つが、v1 では複数人チームではなく1人独自陣営として扱う。
...
- effect 候補
- 終了候補
- winner judgement contribution とその評価結果
- Player の結果候補
...
- first night に自動の inspection result または message を通知する役職は
  `onFirstNightStarted` を override する
...
終了条件も role-owned extension とする。Engine は採用中の各 Role に
`checkEndCondition` を問い合わせ、返された candidate の `sourceRoleId` がその Role
自身であることを検証する。reason は owning Role が定義する opaque な文字列で、
common enum、persistence allowlist、UI switch に列挙しない。
...
- Runtime hook:
  informational-only な `onFirstNightStarted`、`onInspected`、`onAttacked`、`onExecuted`、
...
狂人は `"werewolf"` Team ID を共有していても、`MadmanRole.getActions` が襲撃
action を返さないため、
...
- `onFirstNightStarted`:
  first night 開始時の informational effect だけを返す。許可するのは inspection
  result、public message、private message で、対象選択 action や causal effect ではない。
- `onInspected`:
...
`onFirstNightStarted` から death、protection、`CurrentAction` などの causal effect を
返した場合は contract violation として拒否する。将来、first night 開始時の causal
behavior が必要になった場合は、特定 Role の例外やこの hook の暗黙な拡張ではなく、
独立した generic persisted hook / action-window contract を追加する。
...
`DeathReason` 自体は opaque な文字列。generic な core reason 以外の identifier と
意味は、それを発生させる Role module が所有する。別 Role は自分の reason で同じ
generic death effect と hook pipeline を利用できる。common code と永続層は Role
固有 reason の一覧を持たない。
...
各 winner judgement の id、priority、winner Team、評価は contribution の
`sourceRoleId` が指す Role が所有する。Engine は `resolvedRoleSetup.contributions`
を priority 順に dispatch し、owning Role が返した結果だけを使う。
winner judgement を `resolvedRoleSetup` 内の別 field に複製しない。

winner judgement の identity は `id` 単体ではなく
`(sourceRoleId, id)` の組。judgement ID は Role module 内でだけ一意ならよく、異なる
Role が同じ ID を使っても衝突しない。Registry、resolved setup validator、Engine の
並び順と dispatch はすべてこの組を使い、common code が global namespace を要求しない。
...
- Player ごとの最終結果判定
- resolved action history を見ないと決められない能力制限
...
common engine は、effect、死亡、winner judgement priority の generic な調停だけを行う。
